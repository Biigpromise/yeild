...
CREATE OR REPLACE FUNCTION public.create_campaign_approval_request()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_user_id_created_at ON messages(user_id, created_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_points_desc ON profiles(points DESC) WHERE points > 0;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_tasks_completed ON profiles(tasks_completed DESC) WHERE tasks_completed > 0;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_name_points ON profiles(name, points DESC) WHERE name IS NOT NULL AND name != '';

-- Optimize task-related queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_task_submissions_user_id_status ON task_submissions(user_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_task_submissions_task_id_status ON task_submissions(task_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_task_submissions_status_submitted_at ON task_submissions(status, submitted_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brand_campaigns_brand_id_created_at ON brand_campaigns(brand_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brand_campaigns_status ON brand_campaigns(status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brand_campaigns_admin_approval_status ON brand_campaigns(admin_approval_status) WHERE admin_approval_status = 'pending';

-- Optimize user referrals
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_referrals_referrer_active ON user_referrals(referrer_id, is_active);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_referrals_referred_active ON user_referrals(referred_id, is_active);

-- Optimize brand wallets and transactions
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brand_wallet_transactions_brand_id_created_at ON brand_wallet_transactions(brand_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_brand_wallet_transactions_transaction_type ON brand_wallet_transactions(transaction_type);

-- Optimize admin notifications
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_admin_notifications_is_read_created_at ON admin_notifications(is_read, created_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reward_redemptions_user_id_created_at ON reward_redemptions(user_id, created_at DESC);

-- Phase 3: Optimize frequently used RPC function
...
-- Phase 4: Add missing constraints and improve data integrity
...
-- Phase 5: Create monitoring views for performance tracking
