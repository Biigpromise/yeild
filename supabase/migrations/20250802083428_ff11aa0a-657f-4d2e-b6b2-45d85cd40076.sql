-- Update the get_admin_task_submissions function to include evidence_files array
...
RETURNS TABLE (
  ...
  status text,
  evidence text,
  evidence_files jsonb,  -- Add evidence_files
...
BEGIN
  ...
  SELECT 
    ...
    ts.status,
    ts.evidence,
    ts.evidence_files,  -- Include evidence_files array
    ts.submitted_at,
    ts.reviewed_at,
